...
class TShirtOrder(models.Model):
    ...
    def get_empty_list_help(self, help):
        ...
        content = (
            "People can make orders through the <a href=%(url)s>public page</a>."
            % {"url": url}
        )
        return """
    ...
    def get_statistics(self):
        ...
        new_this_month_domain = expression.AND(
            [this_month_domain, [("state", "=", "new")]]
        )
        nb_new_orders = self.search_count(new_this_month_domain)
        cancelled_this_month_domain = expression.AND(
            [this_month_domain, [("state", "=", "cancelled")]]
        )
        nb_cancelled_orders = self.search_count(cancelled_this_month_domain)
        total_amount = self.read_group(new_this_month_domain, ["amount"], [])[0][
            "amount"
        ]
        total_quantity = self.read_group(this_month_domain, ["quantity"], [])[0][
            "quantity"
        ]
        ...
        return {
            "average_quantity": 0
            if not nb_orders
            else round(total_quantity / nb_orders, 2),
            "average_time": (random.random() * 44)
            + 4,  # simulate a delay between 4 and 48 hours
